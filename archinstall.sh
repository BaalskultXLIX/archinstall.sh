...
arch-chroot /mnt bash -c " ln -sf /usr/share/zoneinfo/Europe/Berlin /etc/localtime;  hwclock --systohc;  echo "de_DE.UTF-8\ UTF-8" >> /etc/locale.gen; locale-gen; echo "de_DE.UTF-8" > /etc/locale.conf; echo "KEYMAP=de-latin1" >  /etc/vconsole.conf; echo "archlinux" >  /etc/hostname; pacman -Syyu networkmanager grub nano efibootmgr plasma plasma-login-manager  acpid avahi iwd fish --noconfirm; systemctl enable acpid avahi-daemon NetworkManager iwd systemd-timesyncd fstrim.timer plasmalogin.service; mkinitcpio -P; passwd; useradd -m -g users -s /bin/fish user; passwd user; usermod -aG wheel,users user; grub-install --target=x86_64-efi --efi-directory=boot/ --bootloader-id=GRUB; grub-mkconfig -o /boot/grub/grub.cfg; exit"
